...
$( "#modal-form" ).on( "submit", function( e ) {
    ...
    $( "#progress" ).show();
    uploadImage().then(function(file) {
        var product;
        ...
        product.save().then( reload ).catch( showError );
    }).catch( showError );
} );
...
Promise.all([
    ...
    if ( !search ) {
        new Parse.Query( "Product" ).include("category").include("supplier").find().then( init ).catch( showError );
    } else {
        ...
        Parse.Query.or( nameQuery, codeQuery, supplierCodeQuery, descriptionQuery ).find().then( init ).catch( showError );
    }
})
.catch( showError );
